...
namespace CalcEngine
{
    ...
    /// <summary>
    /// Subvariable reference.
    /// 求解变量表达式。
    /// </summary>
    class SubVariableExpression : Expression
    {
        CalcEngine _ce;
        Expression _sub;
        static HashSet<string> loopvar = new HashSet<string>();

        public SubVariableExpression(IDictionary<string, object> dct, string name,string formula)
        {
            _ce = new CalcEngine();
            _ce.Variables = dct;
            if (loopvar.Contains(name))
            {
                ...
                foreach (string var in loopvar)
                    sb.Append(string.Format("{0},",var));
                throw new ArgumentException(sb.Append("recycled variable reference.").ToString());
            }
            loopvar.Add(name);
            _sub = _ce.Parse(formula);
            loopvar.Clear();
        }
        public override object Evaluate()
        {
            return _ce.Evaluate(_sub);
        }
    }
    /// <summary>
