...
namespace HGS
{
    class MenuFunction : ContextMenuStrip
    {
        public MenuFunction()
        {
        ...
        const string TESTA =
            @"H	计算水或水蒸汽的焓：p(绝压):MPa,T:℃，计算结果为kJ/kg。	=H(p,T)
            BinTwoThirdsLog	三选二逻辑变换为浮点数：至少两个为真返回1.0，至少两个为假返回0.0.	=BinTwoThirdsLog(logic1,logic2,logic3)
            BinOneLog	单逻辑变换为浮点数：为真返回1.0，为假返回0.0。	=BinOneLog(logic)
            BinbitOflong	判断长整数(UInt64)中的右数从0起第n位:为1返回浮点数1.0，为0返加浮点数0.0。	=BinbitOflong(UInt64,n)
            DMidOfThree	浮点数三取中。	=DMidOfThree(double,double,double)
            LogicbitOflong	判断长整数(UInt64)中的右数从0起第n位:为1返回真，为0返加假，。	=LogicbitOflong(UInt64,n)";
    }
}
