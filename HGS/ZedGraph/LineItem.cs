...
namespace ZedGraph
{
	...
	#region Fields
        ...
        protected Symbol _flag;//Testa 2016-08-12
	...
	#region Properties
        ...
        public Symbol Flag //Testa 2016-08-12
        {
            get { return _flag; }
            set { _flag = value; }
        }
	...
	#region Constructors
        ...
        public LineItem(string label, double[] x, double[] y, Color color, SymbolType symbolType,SymbolType flagType, float lineWidth)
            : this(label, new PointPairList(x, y), color, symbolType, flagType, lineWidth)
		{
		}
		...
		/// type of symbol to use for this <see cref="LineItem"/>.  Use <see cref="SymbolType.None"/>
		/// to hide the symbols.</param>
		public LineItem( string label, double[] x, double[] y, Color color, SymbolType symbolType, SymbolType flagType)
			: this( label, new PointPairList( x, y ), color, symbolType,flagType )
		{
		}
		...
		/// width is scaled based on <see cref="PaneBase.CalcScaleFactor"/>.  Use a value of zero to
		/// hide the line (see <see cref="ZedGraph.LineBase.IsVisible"/>).</param>
		public LineItem( string label, IPointList points, Color color, SymbolType symbolType,SymbolType flagType,  float lineWidth )
			: base( label, points )
		{
            ...
            _symbol.State = Symbol.emState.Normal;
            _flag = new Symbol(flagType,color);
            _flag.State = Symbol.emState.Flag;
		}
		...
		/// type of symbol to use for this <see cref="LineItem"/>.  Use <see cref="SymbolType.None"/>
		/// to hide the symbols.</param>
		public LineItem( string label, IPointList points, Color color, SymbolType symbolType,SymbolType flagType )
			: this( label, points, color, symbolType,flagType, ZedGraph.LineBase.Default.Width )
		{
		}
	...
	#region Methods
		...
		override public void Draw( Graphics g, GraphPane pane, int pos, float scaleFactor  )
		{
			if ( _isVisible )
			{
                ...
                Flag.Draw(g, pane, this, scaleFactor, IsSelected);
			}
		}		
