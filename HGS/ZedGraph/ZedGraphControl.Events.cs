...
namespace ZedGraph
{
	...
	#region Mouse Events
        ...
        private string GetCurBz(DateTime dt)
        {
            DateTime m_BaseDateTime = new DateTime(2011, 12, 6, 8, 0, 0);
            string[] zname = { "一值", "二值", "三值", "四值", "五值", "一值", "二值" };
            ...
            for (int i = 9; i >= 0; i--)
            {
                ...
                if (itmod >= 0 && itmod <= 7)
                    ...
                    return zname[index + 1];
                else if (itmod > 16 && itmod <= 23)
                    return zname[index + 2];
                itmod -= 24;
            }
		...
		protected string MakeValueLabel( Axis axis, double val, int iPt, bool isOverrideOrdinal )
		{
			if ( axis != null )
			{
				...
				{
                    DateTime dt = XDate.XLDateToDateTime(val);
                    //return  XDate.ToString(val, _pointDateFormat);
					return (string.Format("{0}\r\n{1}",XDate.ToString( val, _pointDateFormat ),GetCurBz(dt)));//Testa
				}
        ...
        private Point oldPoint = new Point(-100, -100);
		/// <summary>
        ...
        protected void ZedGraphControl_MouseMove( object sender, MouseEventArgs e )
		{
			if ( _masterPane != null )
			{
                ...
                //*****修正闪烁
				SetCursor( mousePt );
                bool flag = mousePt.Equals(oldPoint);
                oldPoint = mousePt;
                //*****
				...
				else if ( _isShowCursorValues && !flag )
					HandleCursorValues( mousePt );
				else if ( _isShowPointValues && !flag)
					...
					if ( nearestObj is CurveItem && iPt >= 0 )
					{
						...
						else
						{
							...
							else
							{
								...
								//if ( pt.Tag is string )//Testa 2016-07
								...
								//else
								{
                                    ...
                                    this.pointToolTip.SetToolTip( this,  curve.GetYAxis( pane ).Title.Text+ "\r\n" +xStr + "\r\n" + yStr);//Testa 2016-06-15
